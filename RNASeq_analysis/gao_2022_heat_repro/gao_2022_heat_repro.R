...
expression_file <- "data/GSE184983_genes_fpkm_expression.xlsx"

# Read the expression data as tab-delimited (tsv) file
count_data_full <- read_excel(expression_file)
...
# Convert to matrix
count_data <- as.matrix(count_data_full)
...
meta_data$`treatment:ch1` <- gsub("C for 6h", "", meta_data$`treatment:ch1`)   # Remove any 'C'
...
meta_data$fl_id <- meta_data$title %>%
  paste0("count.", .)                   # Add the "count.WT_" prefix
...
fl_count_data <- filtered_counts[, grepl("count.WT", colnames(count_data))]

# Remove extra spaces and convert to numeric while preserving rownames
fl_count_data[] <- apply(fl_count_data, 2, function(x) as.numeric(gsub("^\\s+|\\s+$", "", x)))

# Ensure the entire matrix is numeric by coercing it explicitly to numeric
fl_count_data <- as.data.frame(fl_count_data)  # Convert to data frame to apply numeric coercion
fl_count_data[] <- lapply(fl_count_data, as.numeric)  # Convert each column to numeric

# Verify if conversion is successful
summary(fl_count_data)

# Check for negative values
summary(fl_count_data)
negative_entries <- fl_count_data[rowSums(fl_count_data < 0) > 0, ]
print(negative_entries)

fl_ids <- colnames(fl_count_data)

fl_group <- meta_data[meta_data$fl_id %in% fl_ids, ][['treatment:ch1']]

fl_design <- model.matrix(~fl_group)

# Define the sample information as a data frame
fl_col_data <- data.frame(
  row.names = colnames(fl_count_data),
  condition = fl_group # Modify as per your setup
)

# Convert 'condition' to a factor
fl_col_data$condition <- factor(fl_col_data$condition)

# Create DESeq2 dataset object
fl_dds <- DESeqDataSetFromMatrix(countData = fl_count_data, colData = fl_col_data, design = ~condition)
head(fl_dds)

# Apply DESeq’s normalization
fl_dds <- estimateSizeFactors(fl_dds)

# Inspect normalized counts (optional)
fl_norm_counts <- counts(fl_dds, normalized = TRUE)
formattable(head(fl_norm_counts))

# Run the DESeq2 pipeline
fl_dds <- DESeq(fl_dds)

# Extract results for all genes with default settings
fl_res <- results(fl_dds)

# Order by adjusted p-value
fl_res <- fl_res[order(fl_res$padj),]

#get meaning of the columns
mcols(fl_res, use.names = TRUE)

#other summary
summary(fl_res)

# Filter for significant results (adjusted p-value < 0.05)
fl_deseq2_genes <- subset(fl_res, padj < 0.05)

# Save to file
write.csv(as.data.frame(fl_deseq2_genes), file="output/gao_HS_DESeq2_flower_results.csv")
...
significant_genes <- list(
  DESeq2 = rownames(fl_deseq2_genes))
...
write.csv(cle_all_genes, "cle_all_genes.csv", row.names = FALSE)
write.csv(ralf_all_genes, "ralf_all_genes.csv", row.names = FALSE)


# Expression Matrix Preparation
# Assuming 'dds' is the DESeq2 object and you have a normalized counts matrix
norm_counts <- counts(fl_dds, normalized = TRUE) # Get normalized counts
...
# Normalized counts
norm_counts <- counts(fl_dds, normalized = TRUE)
...
ggsave("plots/gao_ralf_heatmap.pdf", plot = ralf_heatmap[[4]], width = 8, height = 6)
...
# Save the heatmap directly as a PDF
ggsave("plots/gao_cle_heatmap.pdf", plot = cle_heatmap[[4]], width = 8, height = 6)
...
write.csv(cle_all_genes, "RNASeq_analysis/gao_2022_heat_repro/cle_all_genes_with_common_names.csv", row.names = FALSE)

# Save RALF genes with common names to CSV
write.csv(ralf_all_genes, "RNASeq_analysis/gao_2022_heat_repro/ralf_all_genes_with_common_names.csv", row.names = FALSE)
